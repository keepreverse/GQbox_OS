...
/**
 * Возвращает CSS-значение цвета для категории.
 * Источник правды — само поле `category.color` (из БД/JSON).
 * Никаких хардкодов: обновил цвет в словаре → обновился везде.
 */
export function getCategoryColorVar(category: Category | undefined | null): string {
  return category?.color || 'var(--color-accent)';
}
...
/**
 * Возвращает CSS-значение для цвета продукта (material hex).
 * 'gradient' — это маркер кастомного градиентного представления, не сам цвет.
 */

...
export default function Dashboard({ onViewChange, onNavigateToMatrix }: DashboardProps) {
    ...
    const supplierStats = [
      {
        name: 'Angela',
        code: 'A',
        count: supplierCounts[0],
        color: 'var(--color-supplier-a)',
        maxCount: maxSupplierCount,
      },
      {
        name: 'Wendy',
        code: 'W',
        count: supplierCounts[1],
        color: 'var(--color-supplier-w)',
        maxCount: maxSupplierCount,
      },
      {
        name: 'Angela+Wendy',
        code: 'AW',
        count: supplierCounts[2],
        color: 'var(--color-supplier-aw)',
        maxCount: maxSupplierCount,
      },
      {
        name: '—',
        code: '-',
        count: supplierCounts[3],
    ...
    return buckets
      .map((b) => {
        const matching = products.filter(
          (p) => p.powerW != null && p.powerW >= b.min && p.powerW <= b.max
        );
  ...
  const lengthDistribution = useMemo(() => {
    ...
    return buckets
      .map((b) => {
        const matching = products.filter(
          (p) => p.lengthM != null && p.lengthM >= b.min && p.lengthM <= b.max
        );
  ...
  const colorDistribution = useMemo(() => {
    return colors
      .map((c) => ({
        ...
        code: c.code,
        color:
          c.hexValue === 'gradient' ? 'conic-gradient(in hsl longer hue, red, red)' : c.hexValue,
      }))
      .filter((c) => c.value > 0)
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-2">
        <div>
          <h2 className="text-xl sm:text-2xl font-semibold tracking-tight text-gradient">
            {t('dash.title')}
          </h2>
        ...
        <div className="flex items-center gap-2 text-xs text-text-tertiary">
          ...
          <span>
            {t('dash.updated')} {new Date().toLocaleDateString()}
          </span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 items-stretch">
            ...
            >
              ...
              <div className="relative z-10 flex flex-col h-full">
                <div className="flex items-center justify-between min-h-[18px]">
                  ...
                  {data.delta !== 0 && (
                    <span
                      className={`flex items-center gap-0.5 text-[10px] sm:text-xs font-medium px-1.5 py-0.5 rounded-full ${
                        data.trend === 'up'
        ...
        <div className="glass rounded-xl p-4 sm:p-5 relative overflow-hidden group hover:border-border-strong transition-all duration-300 flex flex-col">
          ...
          <div className="relative z-10 flex flex-col h-full">
            <p className="text-[11px] sm:text-xs text-text-tertiary font-medium tracking-wide mb-3 sm:mb-4">
              {t('dash.quickActions')}
            </p>
            <div className="flex flex-col gap-1.5 flex-1 justify-center">
              ...
              >
                ...
                <span className="text-[11px] sm:text-xs text-text-primary truncate">
                ...
                <ResponsiveContainer width="100%" height={260} debounce={16}>
                  ...
                  >
                    ...
                    <Bar
                      ...
                      isAnimationActive={false}
                      onClick={(data: any) =>
                        onNavigateToMatrix?.({ categories: data?.code ? [data.code] : [] })
                      }
        ...
        <div className="glass rounded-xl p-3 sm:p-5 overflow-hidden">
          ...
          <div className="flex flex-wrap gap-x-3 gap-y-1.5 mt-2 justify-center">
            {distributionData.map((d, i) => (
              <div
                key={d.name}
                className="flex items-center gap-1.5 text-[10px] sm:text-[11px] text-text-secondary"
              >
                <div
              ...
              return (
                ...
                >
                  <div
                    ...
                    style={{
                      background: `color-mix(in srgb, ${product.category.color} 8%, transparent)`,
                    }}
                  >
                    <Icon
                      className="w-4 h-4"
        ...
        <div className="glass rounded-xl p-3 sm:p-5">
                ...
                <AreaChart data={stats.supplierStats}>
                  <defs>
                    <linearGradient id="supplierGrad" x1="0" y1="0" x2="1" y2="0">
                      {stats.supplierStats.map((entry, i) => (
                        <stop
                          key={i}
                          offset={`${(i / (stats.supplierStats.length - 1)) * 100}%`}
                  ...
                  <Customized
                    component={(props: any) => {
                      ...
                      const xPos = isVisible
                        ? xAxis.x(hoveredSupplierIndex) - bandwidth / 2
                        : -1000;
                      return (
                        <motion.rect
                          x={xPos}
          ...
          <div className="mt-4 space-y-2">
            ...
            {alerts.map((alert, i) => (
              <div
                key={i}
                className="flex items-center gap-2 min-h-[44px] sm:min-h-0 p-2.5 rounded-lg bg-bg-tertiary/50"
              >
                <AlertCircle
                  className={`w-3.5 h-3.5 flex-shrink-0 ${alert.type === 'warning' ? 'text-warning' : alert.type === 'info' ? 'text-info' : 'text-success'}`}
                />

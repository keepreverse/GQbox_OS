...
import {
  ...
  type InspectorQueryResult,
  type InspectorTableInfo,
...
export function createDevDataSource(): DataSource {
  ...
  const products: ProductsAPI = {
    ...
    getAllMedia() {
      ...
      return rawMediaFiles.map((f) => ({ ...f, linkedSkus: linksByFile.get(f.id) ?? [] }));
    },
    getAllMediaLinks() {
      return [...rawMediaLinks];
    },
    getMediaForVariant(variantId) {
      const links = rawMediaLinks
      ...
      return links.map((l) => {
        const file = rawMediaFiles.find((f) => f.id === l.fileId);
  ...
  return {
    ...
    inspector: {
      ...
      async listTables(): Promise<InspectorTableInfo[]> {
        const res = await request<InspectorTableInfo[]>(`${API_PREFIX}/inspector/tables`);
        return res;
      },
      async dumpTable(table: string): Promise<InspectorQueryResult> {
        const res = await request<InspectorQueryResult>(`${API_PREFIX}/inspector/tables/${table}`);
        return res;
      },
      async runQuery(sql: string): Promise<InspectorQueryResult> {
        const res = await request<InspectorQueryResult>(`${API_PREFIX}/inspector/query`, {
          method: 'POST',
